...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM `election_list` where election_id = '{$_GET['id']}'");
...
<div class="container-fluid">
    <form action="" id="department-form">
        <input type="hidden" name="id" value="<?php echo isset($election_id) ? $election_id : '' ?>">
...
<script>
    $(function(){
        $('#department-form').submit(function(e){
            ...
            $.ajax({
                url:'./../Actions.php?a=save_election',
                method:'POST',
                data:$(this).serialize(),
                dataType:'JSON',
                error:err=>{
                ...
                success:function(resp){
                    if(resp.status == 'success'){
                        ...
                        if("<?php echo isset($election_id) ?>" != 1)
                        _this.get(0).reset();
                    }else{
                        _el.addClass('alert alert-danger')
                    }
